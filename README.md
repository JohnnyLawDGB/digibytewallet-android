...
> **Status:** Active development — **v4.0.83**. BIP157/158 is the only sync path (the bloom wire path was removed in v4.0.0), DigiDollar and Taproot are wired end-to-end, own-node pairing shipped. v4.0.83 makes the confirmation screens show what you will pay and makes Dandelion private send work. A DigiAsset send now shows the fee of the transaction it will actually build — a send that combines many coins used to show a flat estimate far below what it paid — and will not pay more than you approved; a DigiDollar send shows its 0.1 DGB network fee instead of only its currency. Dandelion++ now works after the app restarts, and a stemmed send leaves your phone through its one stem peer only: it used to be announced to other peers as they connected, and a stem the stem peer dropped was never re-sent until the app was restarted. When the peer the wallet syncs from drops while others are still connected, sends keep working instead of being refused until a restart. Funds recovered from another wallet no longer leave the DGB change of a DigiAsset move unspendable. Tapping the DigiDollar balance opens DigiDollar receive, and after the device-lock prompt in Recover funds you return to Recover funds instead of the home screen. Three new native test suites (88 in all) run in CI. v4.0.82 is a second hardening release. Once a wipe has removed the wallet the app starts afresh on onboarding, so the next wallet you create is created and stored cleanly; a wipe that could not finish says so and completes at the next launch. A failure inside the PIN check is reported as such and is never counted as a wrong PIN. Recover funds asks for your device lock when the wallet key needs it, signs an older-style input only with the amount and script its parent transaction proves, and reports the fee it paid. Asset data fetched from a server is accepted only when it is the transaction that was asked for, and an asset send lists every input once and pays the fee it planned; assets are re-identified once on the first launch after the update. A DigiDollar send tries another coin before it gives up. Three new native test suites (85 in all) run in CI. v4.0.81 is a hardening release. The bundled chain and compact-filter checkpoints are refreshed, and every filter checkpoint is now proven against the chain before it ships, so restoring an older wallet no longer stalls part-way through its scan. Data received from the network — transactions, headers, filters and scripts — is handled more strictly, in 32- and 64-bit builds alike. The amount you approve is the amount that is signed everywhere: DigiDollar gains a strict cents parser, the DigiAsset confirmation is written from the parsed quantity, and a lone comma is read as the decimal point in all three. Every DigiAsset output an incoming transfer assigns to you is set aside as soon as it is seen, and again immediately before any send is built; DigiDollar transfers that use any standard script framing are now recognised. Ten new native test suites (82 in all) now run in CI on every push and pull request, and once a week. v4.0.80 stops this wallet destroying a DigiAsset that was issued with transfer rules. Some assets carry a rule — most often a royalty every transfer must pay the issuer — and the wallet, which knew nothing about rules, never built that payment. DigiByte confirmed such a transaction normally, but the DigiAsset layer re-checks the rules afterwards and strips the assets from every output of a transfer that breaks one: not just the amount sent, but the whole holding it came from, the change due back, and any other asset on the same output — while the app reported success and went on showing a balance that existed nowhere. Reported privately by a security researcher, confirmed from source on both sides and then proven on mainnet with throwaway assets on a test device, where sending 2 of 5 units destroyed all 5. An asset the wallet cannot prove is rule-free can now no longer be sent, on the send path or the recovery path, explained on screen in 13 languages, with nothing signed or broadcast; rule-free assets are unaffected. It prevents the loss but cannot undo one — a holding burned by an earlier version may still show units that no longer exist. v4.0.79 makes a scanned payment request carry its amount into the Send screen (it used to arrive with the address only) and converts every DGB amount to satoshis in exact decimal arithmetic — the old floating-point path paid one satoshi short on amounts like 0.29 DGB and turned one tip-wallet dust code in fifteen into a different code; it also reads a lone decimal comma as the decimal point in the seven comma-locale languages (0,29 was being read as 29 DGB), bounds absurd scanned amounts before parsing them, and writes the Receive QR’s amount plainly. v4.0.78 fixes a native crash seen on a Galaxy Ultra where the wallet aborted while replacing its peer manager: peer threads still winding down had the connection state freed underneath them ("destroyed mutex"), so the app died uncleanly and lost its transaction cache — the old peer manager is now parked until its last thread has exited, and the crash is reproduced and gated by a host test. It also stops the wallet reporting itself synced before its scan ledger is safely on disk (a restore could otherwise re-scan from the start after a kill), survives Android refusing a foreground service start when the own-node toggle is flipped from the background, moves DigiAssets whose units cannot be read instead of stranding them during recovery, and adds a privacy toggle that hides balances on the home screen. v4.0.77 completes the Tor loud-fallback — a banner Retry action and a clear degrade when Tor cannot start — and lands a security batch: per-site Digi-ID identity keys so sites can no longer correlate you by address, an auth-bound seed key held in the Android Keystore, and a fix for a device-credential unlock crash on Android 9–10. v4.0.76 closes a lock bypass found by an independent security audit: coming back to the app from the launcher or Recents resumed on the wallet screen with no PIN, so a send — and the DigiAsset, sweep and Hub-login actions — were reachable from a picked-up phone. The app now re-locks on every warm resume, locks after the Security-settings idle timeout (which previously did nothing), asks for the PIN or a fingerprint before every send, Digi-ID approval, sweep, Hub login and node pairing, blocks screenshots and keyboard learning while a recovery phrase or passphrase is typed, stops logging the Hub session token and wallet addresses (the token now lives in encrypted storage), and adds Android 12+ data-extraction rules so no wallet data leaves in an OEM device transfer. v4.0.75 fixes a DigiDollar recovery that would spend an outpoint already gone: the lookup lists every transaction touching an address, spends included, so a receive whose token output was consumed still counted as spendable — three outpoints claimed for two dollars, and a transfer the network rejects. The scan now reads those transactions' inputs to tell a live receive from a spent one; verified on mainnet by moving two dollars in one transaction. v4.0.74 keeps DigiDollar off the "This wallet" recovery tab — those dollars live at m/86' of the seed the wallet already watches, so listing them claimed money was missing when it was in hand, and the sweep would have spent the 0.1 DGB consensus fee sending them to themselves. v4.0.73 fixes a wallet whose only value is DigiDollar reading as empty: a token output carries zero satoshis, so it never landed in the UTXO findings the screen decided from — the dollars were invisible and there was no button to move them. The refusal when there is not enough DGB for the 0.1 DGB consensus fee now reads as a sentence in the user's language rather than a machine constant in satoshis, and a run that broadcast nothing no longer shows a green "Sweep submitted" tick; verified on mainnet, including topping the wallet up and bringing the dollar home. v4.0.72 recovers DigiDollar from another wallet's seed — it lives at m/86' as a zero-value Taproot output, invisible to the ordinary UTXO lookup, so a restored wallet previously got its DGB and assets and was told nothing about its dollars; verified on mainnet. v4.0.71 lets a wallet holding more DigiAssets than spendable outputs move all of them: the recovery splits its DGB first, so 50 assets and one UTXO no longer means moving one asset and stranding forty-nine. v4.0.70 gives a failed send the same full-screen, must-dismiss treatment a successful one gets — it previously reported itself as one line of small red text below the button, which is indistinguishable from nothing having happened. v4.0.69 moves DigiAssets out of a wallet you are recovering from BEFORE sweeping its DGB, which removes the fee estimate the old order needed and with it a whole class of mis-sizing bug; verified on mainnet. v4.0.68 added French (13 languages) and moved the optional BIP39 passphrase onto its own screen after seed verification — storing a passphrase beside the recovery phrase makes it a single secret again, so the two are never recorded in one sitting. v4.0.66 added the optional BIP39 passphrase (12+1 / 24+1) on wallet creation, checked end-to-end against an independent implementation. v4.0.63–v4.0.65 make the wallet speak 12 languages — German, Spanish, Hindi, Chinese, Japanese, Portuguese (BR), Indonesian, Vietnamese, Turkish, Russian and Filipino alongside English — with a language picker on the first screen, before any wallet is created. **[Download APK](https://digiscope.me/downloads/)** · **[Latest GitHub release](https://github.com/JohnnyLawDGB/digibytewallet-android/releases/latest)**
...
**~1,690 unit tests, 0 failures**, including 50+ dedicated security tests covering:
