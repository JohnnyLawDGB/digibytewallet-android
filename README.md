...
> **Status:** Active development — **v4.0.42**. BIP157/158 is the only sync path (the bloom wire path was removed in v4.0.0), DigiDollar and Taproot are wired end-to-end, own-node pairing shipped. v4.0.42 fixes a send that could report success while the network had rejected it — which left the wallet holding coins that existed nowhere else — and can now clear the stranded transactions that resulted. **[Download APK](https://digiscope.me/downloads/)** · **[Latest GitHub release](https://github.com/JohnnyLawDGB/digibytewallet-android/releases/latest)**
