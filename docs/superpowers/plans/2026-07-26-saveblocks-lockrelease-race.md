...
- **Class audit — scoped to the PEER-MANAGER block-persistence path (BRPeerManager.c, all 9 callback fields swept + hostile-review-confirmed):** ONLY `saveBlocks@1586` has the defect. `savePeers@1063` is the SAFE TEMPLATE (by-value `BRPeer save[]` snapshot under the lock — but `BRPeer` is a flat struct; `BRMerkleBlock` is NOT, it has `hashes`/`flags` heap pointers, so a shallow snapshot is insufficient — hence serialize, not copy). `saveFilterHeaders@2389` is a NEAR-MISS: passes `manager->compactFilterChain` but is safe ONLY because the lock is still held (unlock at `:2458`) — Task 3 pins that invariant. Every other callback passes only the opaque `manager->info` or a freshly-malloc'd buffer.
- **KNOWN SIBLING outside this scope (hostile review, filed as FOLLOW-UP — NOT fixed here):** `BRWallet.c` has the structurally identical shape: `BRWalletRegisterTransaction` releases `wallet->lock` then fires `wallet->balanceChanged(...)` + `wallet->txAdded(callbackInfo, tx)` (BRWallet.c:1633-1634) and `txUpdated` (1866/1890) with a live `BRTransaction *tx` that another thread can free under `wallet->lock` (`BRWalletRemoveTransaction` ~1641). Same defect class, DIFFERENT subsystem + structure (`BRTransaction` lifetime is more involved than a block's), NO crash record here (it is the upstream breadwallet dispatch pattern). This spec deliberately does NOT expand to it — separate bug, separate spec/review/TSan gate (see [[project_cf_parser_overrun_security]] discipline). Recorded so "fix the class" isn't silently over-claimed; the wallet-callback sibling is its own future item.
- **Hold-time arithmetic (option (a) chosen):** `saveCount` hard-capped at `SAVE_BLOCK_COUNT=300` (BRPeerManager.h:60). CF-only path ⇒ every saved block is a bare **80-byte** header (`totalTx=0`, `hashes/flags=NULL`; blocks enter only via `_BRPeerAcceptHeadersMessage`→`BRMerkleBlockParse(…,81)` where the `off+4<=bufLen` guard fails ⇒ `totalTx=0`). Worst realistic save = **300 × 80 B = 24 KB**, added lock hold **~25-45 µs** on a Note 8; bounded ≤ ~150 µs even in the one-time post-upgrade case (legacy `totalTx>0` merkleblocks ~730 B, which `ClearMemory` evicts within a few save cycles). **Invisible to #33:** the UI never acquires `manager->lock`; it reads cached status mirrors refreshed under the lock at `~:1577`. Option (b) snapshot-then-serialize buys only ~10-15 µs of lock window at the cost of a **double copy** ⇒ rejected. (The real discriminator is single-copy (a) vs double-copy (b): option (a) ALSO malloc's its ~24 KB framed buffer under the lock, so "malloc-under-lock" is NOT the differentiator — the single 24 KB malloc doesn't take `manager->lock` and is sub-µs.)
...
**Design:** `#include "BRPeerManager.c"`. Two pthreads sharing one `BRPeerManager`: (1) a **relay thread** that repeatedly drives the save path (fill `saveBlocks[]` from `manager->blocks`, unlock, then serialize the live pointers), and (2) a **mutation thread** that in a tight loop churns `manager->blocks` — insert/`BRSetRemove`+`BRMerkleBlockFree` (reorg), including at least one `totalTx>0` block with malloc'd `hashes`/`flags` so a freed-hashes memcpy is reachable. Deliberately target the unlock→callback window. Build `run.sh` on `gcs_match_kat/run.sh`'s unit list + **`-fsanitize=thread -fno-omit-frame-pointer -g`** (NOT ASan — this is a data race, TSan's domain).

**⚠️ HARNESS TRAP (review, must obey or the RED gate is worthless):** do NOT register the real `bridge_saveBlocks` as the `saveBlocks` callback — it early-returns when there is no `JNIEnv` (jni_peer.c:244-248), which is exactly the host case, so the racy serialize would never run and the harness would be **green on unfixed code = false pass.** The harness MUST register a **custom serialize-replica callback** that reproduces jni_peer.c's two-pass `BRMerkleBlockSerialize` over the live `blocks[]` pointers AFTER the unlock and skips the JNI upcall — that replica is what must race the mutation thread. (First threaded/TSan KAT in the tree — `run-host-kats.sh` only checks exit codes so a TSan `run.sh` fits, but confirm the TSan runtime is available on the CI host before relying on it as a gate; if not, the harness runs locally and the CI gate is the ASan suite + on-device.)
...
**Files:** Modify submodule `BRPeerManager.c` (`_peerRelayedBlock` + the `saveBlocks` fn-ptr field + **`BRPeerManagerSetCallbacks`** param — NOT `BRPeerManagerNew`, which takes no callbacks) and outer `jni_peer.c` (`bridge_saveBlocks` + the `BRPeerManagerSetCallbacks` registration at ~:885). Keep the on-wire persistence format byte-identical (Kotlin's parser is unchanged).
...
- [ ] **Step 2:** Update the `saveBlocks` fn-ptr typedef in the `BRPeerManager` struct (`:218`) + the **`BRPeerManagerSetCallbacks`** signature (`:2611`) + its `.h` (`:145`). Update `jni_peer.c`: register the new-signature callback (`:885`); `bridge_saveBlocks` (`:237`) now receives `(bytes,len)` and does only the JNI upcall. Also: correct the stale `.h:190` / `.c:2605-2610` doc comments that claim a peer disconnect can fire `saveBlocks` (false in this codebase — `:1586` is the only caller), and add a one-line footnote that `native/src/main/jni/bridge/PeerManager.c` holds an old-ABI `saveBlocks` copy but is **not compiled** (CMakeLists.txt:118) — a future re-enable must adopt the new ABI.
