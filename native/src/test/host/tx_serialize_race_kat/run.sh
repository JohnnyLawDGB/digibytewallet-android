...
# (jni_transaction_persist.c). SAME CLASS as the saveBlocks race
# (saveblocks_race_kat), on the transaction-persist path.
#
# Drives the REAL BRWallet/BRTransaction primitives in a bounded 2-thread ASan
# race that mirrors Java_..._getSerializedTransactions (see the _main.c header).
# The script builds BOTH code shapes and REQUIRES:
#   * TXSERIALIZE_FIXED=0 (snapshot-then-unlock-then-serialize, the shipped shape)
#       -> ASan heap-use-after-free (RED)
#   * TXSERIALIZE_FIXED=1 (serialize-under-lock via BRWalletSerializeTransactions)
#       -> clean exit 0 (GREEN)
# The KAT FAILS if the unfixed pattern does NOT fault — a gate that can't detect
# the bug is worthless (red-before-green, enforced structurally).
#
# ASan (not TSan): the defect is a use-after-free on a freed BRTransaction's
...
# report for tens of seconds; we only need to DETECT the fault, not name frames.
...
if grep -qiE "heap-use-after-free|AddressSanitizer" "$BUILD_DIR/unfixed.out"; then
    echo "RED confirmed: unfixed pattern faults (exit $RED_EXIT):"
    grep -iE "ERROR: AddressSanitizer|heap-use-after-free|READ of size|#0 " "$BUILD_DIR/unfixed.out" | head -4
else
...
if ! build 1 "$BUILD_DIR/fixed"; then echo "FAIL: fixed build error (BRWalletSerializeTransactions missing?)"; exit 1; fi
...
echo "tx_serialize_race_kat: PASS (RED on unfixed pattern, GREEN on fixed pattern)"
exit 0
