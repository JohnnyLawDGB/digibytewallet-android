...
If no PIN hash is on disk the app routes to PIN setup over the existing seed rather than demanding the seed phrase. An attacker reaches that state only by deleting `dgb_pin_store`, which on release builds needs root: `isDebuggable=false` kills `run-as`, `allowBackup=false` kills `adb backup`, and `dataExtractionRules` exclude the store from cloud backup and device transfer. Root already defeats every control above. The same branch is also reachable without an attacker: `AppNavigation` treats any exception from `pinManager.hasPin()` as "no PIN" (`AppNavigation.kt` `hasPin` try/catch), so a Keystore eviction of the `dgb_pin_store` master key (OEM keystore reset) while the seed alias survives routes to PIN setup over the live seed — a device-fault availability path, not an attacker-reachable one. Both are recorded rather than closed.
