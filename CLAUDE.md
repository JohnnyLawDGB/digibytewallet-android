...
**Version:** v4.0.78 (versionCode 40078)  _(source of truth: `app/build.gradle.kts` `versionName`/`versionCode` — bump there on release and mirror here)_
...
- **Peer tiering (the canon):** the wallet's ONLY good CF peers are, in order: (1) the **15 hardcoded CF oracle nodes** in the shared core's `BRPeerCanon.h` (moved out of `jni_peer.c` 2026-09-05 so iOS gets the same table; `BR_PEER_CANON_SERVICES` requires COMPACT_FILTERS; the host KAT enforces IPv4-literals-only, no DNS on the bootstrap path) — the **CANON**; (2) the capability-aware **seeder crawler** `api.digiscope.me/api/peers` (returns filter-validated `/9.26.4/` peers, uptime/composite-scored) — the **FALLBACK only when the canon fails**. The general DHT/addr-relayed pool is **JUNK** (old `/7.17//8.22/` nodes tens-of-thousands of blocks behind → rejected `node isn't synced`; non-SPV `/8.26/` → `doesn't support SPV mode`). **The wallet must PREFER/PIN the canon and NOT roam the junk pool.** Today it injects the 15 + seeder but ALSO dials the junk pool and roams without pinning — the gap behind fragile real-time sync. Rescan pins the priority peer (`BRPeerManagerSetFixedPeer`, line 63) which is exactly why rescan is robust and real-time roaming is not.
